...
function draw() {
        for (let i = 0; i < ballcount; i++) {
                for(let j = 0; j < ballcount; j++){
                        if(i != j){
                                sx[i] += size[j]*size[j]*(x[j]-x[i])/((x[i]-x[j])*(x[i]-x[j])+(y[i]-y[j])*(y[i]-y[j]))/Math.sqrt((x[i]-x[j])*(x[i]-x[j])+(y[i]-y[j])*(y[i]-y[j]));
                                sy[i] += size[j]*size[j]*(y[j]-y[i])/((x[i]-x[j])*(x[i]-x[j])+(y[i]-y[j])*(y[i]-y[j]))/Math.sqrt((x[i]-x[j])*(x[i]-x[j])+(y[i]-y[j])*(y[i]-y[j]));
                        }
                }
